...
export const USER_DELETE_URL =
  new InjectionToken<string>('delete user API');
...
export class AuthService {
  ...
  constructor(
    ...
    @Inject(USER_DELETE_URL)
    private delete_user_url: string,
  ...
  deleteUser() {
    ...
    return this.http
      .post(this.delete_user_url, user)
      .pipe(
        switchMap(() => {
          this.userInfoFireService.deleteUserData(
            this._userChange
          );
  ...
  updateEmail(newEmail: string) {
    return from(this.auth.currentUser).pipe(
      tap((user) => user.updateEmail(newEmail))
    );
  }
}
