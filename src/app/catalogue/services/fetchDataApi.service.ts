...
export class FetchDataApi {
  ...
  getCountries(filmData: MovieApiResult) {
    const countries =
      filmData.Country.split(', ');
    return forkJoin(
      countries.map((countryCode) => {
        if (countryCode.includes(' ')) {
          return null;
        } else {
          return this.apiService.getCountryByCode(
            countryCode
          );
        }
      })
  ...
  mapToFireData(
    filmData: MovieApiResult,
    bookData: BookApiResult
  ) {
    if (filmData.Response == 'True') {
      return this.getCountries(filmData).pipe(
        map<CountryApiResult[], Country[]>(
          (element) => {
  ...
  mapFireDataModel(
    ...
    return {
      ...
      authors: book?.items[0].volumeInfo?.authors
        ...
        : null,
      categories: book?.items[0].volumeInfo
        ?.categories
